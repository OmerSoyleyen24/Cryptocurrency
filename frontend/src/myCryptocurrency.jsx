...
function MyCryptocurrency() {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        const res = await fetch(`http://localhost:8001/api/auth/users/${username}/cryptos`);
  ...
  const saveToDb = async (listToSave) => {
    ...
    try {
      await fetch(`http://localhost:8001/api/auth/users/${username}/update-cryptos`, {
        method: "PUT",
  ...
  const runPrediction = async (id) => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:8002/api/ml/predict?${query}`);
      const data = await res.json();
      ...
      setCryptos(prev => prev.map(c => 
        c.id === id ? { 
          ...
          imageUrl: `http://localhost:8002${data.graph}`,          
          predictionInfo: {
  ...
  const runRagSearch = async () => {
    ...
    setRagLoading(true);
    try {
      const res = await fetch(`http://localhost:8003/api/rag/search`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
